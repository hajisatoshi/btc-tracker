...
class APIClient:
    ...
    def get_transactions(self, transaction_type: str = None) -> tuple:
...
class LoginFrame:
    ...
    def create_frame(self):
        ...
        # Title
        title_label = tk.Label(self.frame, text="BTC Portfolio Tracker", 
...
class SummaryTab:
    ...
    def update_display(self):
        ...
        net_cost_usd = data.get('net_cost_basis_usd', 0)
        net_cost_cad = data.get('net_cost_basis_cad', 0)
        current_value_usd = data.get('current_value_usd')
        current_value_cad = data.get('current_value_cad')
        
        self.net_btc_label.config(text=f"Net BTC: {format_btc_amount(net_btc)}")
        self.net_cost_label.config(text=f"Net Cost Basis: {format_currency(net_cost_usd)} / {format_currency(net_cost_cad, 'CAD')}")
        
        if current_value_usd is not None:
            self.current_value_label.config(
                text=f"Current Value: {format_currency(current_value_usd)} / {format_currency(current_value_cad, 'CAD')}")
            
            # Calculate gain/loss
            gain_loss_usd = current_value_usd - net_cost_usd
            gain_loss_cad = current_value_cad - net_cost_cad
            
            color = "#2ECC71" if gain_loss_usd >= 0 else "#E74C3C"
            sign = "+" if gain_loss_usd >= 0 else ""
            
            self.gain_loss_label.config(
                text=f"Gain/Loss: {sign}{format_currency(gain_loss_usd)} / {sign}{format_currency(gain_loss_cad, 'CAD')}",
                fg=color)
        else:
        ...
        self.savings_cost_label.config(text=f"Cost: {format_currency(savings.get('cost_usd', 0))}")
        
        self.spending_btc_label.config(text=f"BTC: {format_btc_amount(spending.get('btc', 0))}")
        self.spending_cost_label.config(text=f"Cost: {format_currency(spending.get('cost_usd', 0))}")
        
        # Price
        btc_price_usd = data.get('btc_price_usd')
        btc_price_cad = data.get('btc_price_cad')
        
        if btc_price_usd is not None:
            self.btc_price_label.config(
                text=f"BTC Price: {format_currency(btc_price_usd)} / {format_currency(btc_price_cad, 'CAD')}")
        else:
...
class TransactionTab:
    ...
    def create_transaction_form(self, parent):
        ...
        currency_frame.grid(row=1, column=1, sticky="w", pady=(10, 0))
        ...
        # Cost
        tk.Label(fields_frame, text="Cost:", font=("Arial", 10)).grid(row=1, column=2, sticky="w", padx=(0, 10), pady=(10, 0))
        self.cost_entry = tk.Entry(fields_frame, width=15, font=("Arial", 10))
        self.cost_entry.grid(row=1, column=3, padx=(0, 20), pady=(10, 0))
    ...
    def refresh_transactions(self):
        ...
        try:
            response, status_code = self.api_client.get_transactions(self.transaction_type)
    ...
    def display_transactions(self):
        ...
        for transaction in self.transactions:
            cost_display = CurrencyConverter.convert_from_usd(transaction["cost_usd"], currency)
...
3. Example CSV format:
   ...
   2024-02-01,spend,0.00050000,25.00,33.75,Paid for coffee
""")
